...
from sqlalchemy import Enum as SAEnum, Uuid, text, Boolean, DateTime, Float, ForeignKey, Integer, String, Text
...
class Lead(Base):
    ...
    source: Mapped[LeadSource] = mapped_column(SAEnum(LeadSource, native_enum=False), nullable=False, default=LeadSource.INBOUND)
    status: Mapped[LeadStatus] = mapped_column(SAEnum(LeadStatus, native_enum=False), nullable=False, default=LeadStatus.NEW, index=True)
    score: Mapped[int] = mapped_column(Integer, default=0)
    industry: Mapped[IndustryVertical | None] = mapped_column(SAEnum(IndustryVertical, native_enum=False), nullable=True)
    employee_count: Mapped[int | None] = mapped_column(Integer, nullable=True)
    annual_revenue: Mapped[float | None] = mapped_column(Float, nullable=True)
    notes: Mapped[str | None] = mapped_column(Text, nullable=True)
    assigned_to: Mapped[str | None] = mapped_column(Uuid, nullable=True)
    last_contacted_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    next_followup_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    meta_data: Mapped[dict | None] = mapped_column(JSONBCompat, nullable=True)
...
class PipelineDeal(Base):
    ...
    lead_id: Mapped[str] = mapped_column(Uuid, ForeignKey("leads.id"), nullable=False, index=True)
    org_id: Mapped[str | None] = mapped_column(Uuid, nullable=True, index=True)
    name: Mapped[str] = mapped_column(String(256), nullable=False)
    stage: Mapped[PipelineStage] = mapped_column(SAEnum(PipelineStage, native_enum=False), nullable=False, default=PipelineStage.PROSPECTING, index=True)
    status: Mapped[DealStatus] = mapped_column(SAEnum(DealStatus, native_enum=False), nullable=False, default=DealStatus.OPEN, index=True)
...
class DealActivity(Base):
    ...
    deal_id: Mapped[str] = mapped_column(Uuid, ForeignKey("pipeline_deals.id"), nullable=False, index=True)
    activity_type: Mapped[str] = mapped_column(String(100), nullable=False)
    description: Mapped[str] = mapped_column(Text, nullable=False)
    outcome: Mapped[str | None] = mapped_column(String(200), nullable=True)
    performed_by: Mapped[str] = mapped_column(Uuid, nullable=False)
...
class EnterpriseDemo(Base):
    ...
    lead_id: Mapped[str | None] = mapped_column(Uuid, ForeignKey("leads.id"), nullable=True)
    deal_id: Mapped[str | None] = mapped_column(Uuid, ForeignKey("pipeline_deals.id"), nullable=True)
    title: Mapped[str] = mapped_column(String(256), nullable=False)
    scenario: Mapped[str] = mapped_column(String(100), nullable=False)
    scheduled_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, index=True)
    completed_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    status: Mapped[str] = mapped_column(String(50), nullable=False, default="scheduled")
    presenter_id: Mapped[str] = mapped_column(Uuid, nullable=False)
    attendees: Mapped[list[str]] = mapped_column(JSONBCompat, default=list)
    custom_data: Mapped[dict | None] = mapped_column(JSONBCompat, nullable=True)
...
class CustomerOnboarding(Base):
    ...
    org_id: Mapped[str] = mapped_column(Uuid, nullable=False, unique=True, index=True)
    deal_id: Mapped[str | None] = mapped_column(Uuid, ForeignKey("pipeline_deals.id"), nullable=True)
    current_stage: Mapped[OnboardingStage] = mapped_column(SAEnum(OnboardingStage, native_enum=False), nullable=False, default=OnboardingStage.SIGNED, index=True)
    assigned_csm: Mapped[str | None] = mapped_column(Uuid, nullable=True)
    started_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=_utcnow, server_default=text("CURRENT_TIMESTAMP"))
    completed_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    target_go_live: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    stage_data: Mapped[dict] = mapped_column(JSONBCompat, default=dict)
    blockers: Mapped[list[dict]] = mapped_column(JSONBCompat, default=list)
...
class OnboardingTask(Base):
    ...
    onboarding_id: Mapped[str] = mapped_column(Uuid, ForeignKey("customer_onboarding.id"), nullable=False, index=True)
    stage: Mapped[OnboardingStage] = mapped_column(SAEnum(OnboardingStage, native_enum=False), nullable=False)
    title: Mapped[str] = mapped_column(String(256), nullable=False)
...
class CustomerHealth(Base):
    ...
    status: Mapped[CustomerHealthStatus] = mapped_column(SAEnum(CustomerHealthStatus, native_enum=False), nullable=False, default=CustomerHealthStatus.HEALTHY, index=True)
    ...
    last_calculated: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=_utcnow, server_default=text("CURRENT_TIMESTAMP"))
    risk_factors: Mapped[list[dict]] = mapped_column(JSONBCompat, default=list)
    positive_signals: Mapped[list[dict]] = mapped_column(JSONBCompat, default=list)
...
class RetentionWorkflow(Base):
    ...
    started_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=_utcnow, server_default=text("CURRENT_TIMESTAMP"))
    completed_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    actions: Mapped[list[dict]] = mapped_column(JSONBCompat, default=list)
    outcome: Mapped[str | None] = mapped_column(String(100), nullable=True)
    meta_data: Mapped[dict | None] = mapped_column(JSONBCompat, nullable=True)
...
class EnterprisePricing(Base):
    ...
    org_id: Mapped[str] = mapped_column(Uuid, nullable=False, unique=True, index=True)
    plan_id: Mapped[str] = mapped_column(Uuid, ForeignKey("subscription_plans.id"), nullable=True)
    ...
    billing_interval: Mapped[BillingInterval] = mapped_column(SAEnum(BillingInterval, native_enum=False), default=BillingInterval.ANNUAL)
    ...
    custom_features: Mapped[list[str]] = mapped_column(JSONBCompat, default=list)
    discount_percent: Mapped[float] = mapped_column(Float, default=0.0)
    negotiated_by: Mapped[str | None] = mapped_column(Uuid, nullable=True)
    notes: Mapped[str | None] = mapped_column(Text, nullable=True)
    meta_data: Mapped[dict | None] = mapped_column(JSONBCompat, nullable=True)
...
class UsageBasedBilling(Base):
    ...
    breakdown: Mapped[dict] = mapped_column(JSONBCompat, default=dict)
    invoice_id: Mapped[str | None] = mapped_column(Uuid, nullable=True)
    status: Mapped[str] = mapped_column(String(50), default="pending")
...
class IndustrySolution(Base):
    ...
    industry: Mapped[IndustryVertical] = mapped_column(SAEnum(IndustryVertical, native_enum=False), nullable=False, unique=True, index=True)
    name: Mapped[str] = mapped_column(String(200), nullable=False)
    description: Mapped[str] = mapped_column(Text, nullable=False)
    key_problems: Mapped[list[str]] = mapped_column(JSONBCompat, default=list)
    key_use_cases: Mapped[list[dict]] = mapped_column(JSONBCompat, default=list)
    required_agents: Mapped[list[str]] = mapped_column(JSONBCompat, default=list)
    required_connectors: Mapped[list[str]] = mapped_column(JSONBCompat, default=list)
    compliance_requirements: Mapped[list[str]] = mapped_column(JSONBCompat, default=list)
    demo_scenarios: Mapped[list[dict]] = mapped_column(JSONBCompat, default=list)
    roi_metrics: Mapped[list[dict]] = mapped_column(JSONBCompat, default=list)
    case_studies: Mapped[list[dict]] = mapped_column(JSONBCompat, default=list)
    pricing_guidance: Mapped[dict | None] = mapped_column(JSONBCompat, nullable=True)
...
class Partner(Base):
    ...
    name: Mapped[str] = mapped_column(String(200), nullable=False)
    partner_type: Mapped[PartnershipType] = mapped_column(SAEnum(PartnershipType, native_enum=False), nullable=False, index=True)
    description: Mapped[str | None] = mapped_column(Text, nullable=True)
...
class PartnershipIntegration(Base):
    ...
    partner_id: Mapped[str] = mapped_column(Uuid, ForeignKey("partners.id"), nullable=False, index=True)
    ...
    data_mapping: Mapped[dict | None] = mapped_column(JSONBCompat, nullable=True)
...
class MarketOpportunity(Base):
    ...
    signals: Mapped[list[dict]] = mapped_column(JSONBCompat, default=list)
    recommended_approach: Mapped[str | None] = mapped_column(Text, nullable=True)
    competitive_landscape: Mapped[list[dict]] = mapped_column(JSONBCompat, default=list)
    status: Mapped[str] = mapped_column(String(50), default="identified")
    assigned_to: Mapped[str | None] = mapped_column(Uuid, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=_utcnow, server_default=text("CURRENT_TIMESTAMP"))
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), onupdate=_utcnow)
...
class SalesPrediction(Base):
    ...
    factors: Mapped[list[dict]] = mapped_column(JSONBCompat, default=list)
...
class CaseStudy(Base):
    ...
    org_id: Mapped[str] = mapped_column(Uuid, nullable=False, index=True)
    title: Mapped[str] = mapped_column(String(256), nullable=False)
    industry: Mapped[IndustryVertical] = mapped_column(SAEnum(IndustryVertical, native_enum=False), nullable=False, index=True)
    ...
    key_results: Mapped[list[dict]] = mapped_column(JSONBCompat, default=list)
    ...
    agents_used: Mapped[list[str]] = mapped_column(JSONBCompat, default=list)
    connectors_used: Mapped[list[str]] = mapped_column(JSONBCompat, default=list)
...
class ROICalculator(Base):
    ...
    inputs: Mapped[dict] = mapped_column(JSONBCompat, nullable=False)
    results: Mapped[dict] = mapped_column(JSONBCompat, nullable=False)
    assumptions: Mapped[dict] = mapped_column(JSONBCompat, default=dict)
    calculated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=_utcnow, server_default=text("CURRENT_TIMESTAMP"))
    meta_data: Mapped[dict | None] = mapped_column(JSONBCompat, nullable=True)
...
class CustomerSuccessReport(Base):
    ...
    period_end: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    health_score: Mapped[int] = mapped_column(Integer, default=100)
    usage_summary: Mapped[dict] = mapped_column(JSONBCompat, default=dict)
    adoption_metrics: Mapped[dict] = mapped_column(JSONBCompat, default=dict)
    business_impact: Mapped[dict] = mapped_column(JSONBCompat, default=dict)
    recommendations: Mapped[list[dict]] = mapped_column(JSONBCompat, default=list)
    risks: Mapped[list[dict]] = mapped_column(JSONBCompat, default=list)
    next_steps: Mapped[list[dict]] = mapped_column(JSONBCompat, default=list)
    generated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=_utcnow, server_default=text("CURRENT_TIMESTAMP"))
    meta_data: Mapped[dict | None] = mapped_column(JSONBCompat, nullable=True)
...
class BusinessImpactMeasurement(Base):
    ...
    measured_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=_utcnow, server_default=text("CURRENT_TIMESTAMP"))
    attribution: Mapped[list[dict]] = mapped_column(JSONBCompat, default=list)
    meta_data: Mapped[dict | None] = mapped_column(JSONBCompat, nullable=True)
